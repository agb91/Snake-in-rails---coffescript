...
(function() {
  var abbassaTime, altezzaColonna, aumentaPunti, barando, checkAlive, checkFood, clean, color, death, direzione, downkey, draw, eat, food, forward, forwardDown, forwardLeft, forwardRight, forwardUp, fuori, getColor, getLabVector, getLabVector1, getLabVector2, getOpacity, getRandomInt, gnam, goon, hidden, keyb, larghezzaRiga, leftkey, leggiPunti, morto, moveDown, moveLeft, moveRight, moveUp, nero, punti, readJSON, rightkey, round, serpente, setLabirinth, shake, shakeHorizontal, shakeVertical, shakera, time, unHide, upkey, writeLabirinth;
  ...
  round = function(id) {
    var cid;
    cid = '#' + id;
    return $(cid).css('border-radius', '20px');
  };
  ...
  setLabirinth = function(l) {
    var bars, direction, i, j, length, livello, ostacolo, ref, results, xstart, ystart;
    bars = l.length;
    results = [];
    for (i = j = 0, ref = bars - 1; 0 <= ref ? j <= ref : j >= ref; i = 0 <= ref ? ++j : --j) {
      livello = l[i].level;
      xstart = l[i].xstart;
      ystart = l[i].ystart;
      length = l[i].length;
      ...
      results.push(writeLabirinth(ostacolo));
    }
    return results;
  };

  writeLabirinth = function(ostacolo) {
    var i, j, ref, results;
    results = [];
    for (i = j = 0, ref = ostacolo.length - 1; 0 <= ref ? j <= ref : j >= ref; i = 0 <= ref ? ++j : --j) {
      results.push(color('blue', ostacolo[i]));
    }
    return results;
  };

  getLabVector = function(livello, xstart, ystart, length, direction) {
    var ostacolo;
    if (parseInt(direction) === 2) {
      ostacolo = getLabVector2(xstart, ystart, length);
    } else {
      if (parseInt(direction) === 1) {
        ostacolo = getLabVector1(xstart, ystart, length);
      }
    }
    return ostacolo;
  };

  getLabVector2 = function(xstart, ystart, length) {
    var i, id, j, os2, ref;
    os2 = [];
  ...
  getLabVector1 = function(xstart, ystart, length) {
    var i, id, j, os1, ref;
    os1 = [];
    for (i = j = 0, ref = length; 0 <= ref ? j <= ref : j >= ref; i = 0 <= ref ? ++j : --j) {
  ...
  readJSON = function(livello) {
    return $.getJSON('/get_labi.json', function(data) {
      var a, attuale, i, j, n, ref, ris;
      a = data.get_labi.length;
      ris = [];
      n = 0;
      for (i = j = 0, ref = a - 1; 0 <= ref ? j <= ref : j >= ref; i = 0 <= ref ? ++j : --j) {
      ...
      return setLabirinth(ris);
    });
  };

  leggiPunti = function() {
    return punti = $('#actualRecord').text().trim();
  };

  unHide = function() {
    var c, j, k, m, o, p, r, s, t, u, v, w, x, y;
    hidden = hidden + 1;
    if (hidden === 1) {
      for (c = j = 32; j <= 37; c = ++j) {
        for (r = k = 15; k <= 32; r = ++k) {
  ...
  shake = function(a, dir) {
    var i, j, k, m, o, results;
    if (dir === 'v') {
      for (i = j = 0; j <= 32; i = ++j) {
        moveDown('#' + i + 'u' + a);
      }
    ...
    if (dir === 'h') {
      ...
      results = [];
      for (i = o = 0; o <= 32; i = ++o) {
        results.push(moveLeft('#' + a + 'u' + i));
      }
      return results;
    }
  };

  shakeVertical = function() {
    shake(1, 'v');
    shake(3, 'v');
    shake(8, 'v');
    shake(11, 'v');
  ...
  window.avvia = function(l) {
    var run;
    leggiPunti();
    readJSON(l);
    draw();
    food();
    return run = setInterval(forward, time);
  };
  ...
  food = function() {
    var c, l, opa, r1, r2, r3, rc, ris, rr;
    ...
    if (r1 === 255 && r2 === 255 && r3 === 0 && opa === 1) {
      return color('red', rr + 'u' + rc);
    } else {
      return food();
    }
  };

  checkAlive = function(id) {
    var alive, c, l, r1, r2, r3, ris;
    alive = true;
    c = getColor(id);
    l = c.length;
    ris = c.substring(4, l - 1).split(',');
  ...
  death = function() {
    return window.location.href = '/postmortem/index?data=' + punti;
  };

  checkFood = function(id) {
    var c, f, l, r1, r2, r3, ris;
    f = false;
    c = getColor(id);
    l = c.length;
    ris = c.substring(4, l - 1).split(',');
  ...
  aumentaPunti = function(q) {
    punti = parseInt(punti) + parseInt(q);
    return $('#actualRecord').text(punti);
  };

  goon = function(nuovatesta) {
    var dim, i, j, k, len, len1, m, o, ref, ref1, results, results1, ri, ro, serpenteNew;
    serpenteNew = [];
    dim = serpente.length;
    serpenteNew[0] = nuovatesta;
    ro = (function() {
      results = [];
      for (var j = 1, ref = dim - 1; 1 <= ref ? j <= ref : j >= ref; 1 <= ref ? j++ : j--){ results.push(j); }
      return results;
    }).apply(this);
    for (k = 0, len = ro.length; k < len; k++) {
      i = ro[k];
      serpenteNew[i] = serpente[i - 1];
    }
    if (gnam(nuovatesta)) {
      serpenteNew[dim] = serpente[dim - 1];
    }
    ri = (function() {
      results1 = [];
      for (var m = 0, ref1 = serpenteNew.length - 1; 0 <= ref1 ? m <= ref1 : m >= ref1; 0 <= ref1 ? m++ : m--){ results1.push(m); }
      return results1;
    }).apply(this);
    for (o = 0, len1 = ri.length; o < len1; o++) {
      i = ri[o];
  ...
  forward = function() {
    ...
    if (checkAlive(nuovatesta)) {
      serpente = goon(nuovatesta);
    }
    return draw();
  };

  clean = function(id) {
    return color('yellow', id);
  };
...
$('#cssmenu').prepend('<div id="bg-one"></div><div id="bg-two"></div><div id="bg-three"></div></div>');
});
} )( jQuery );
